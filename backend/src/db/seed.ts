...
export const seedDatabase = async (): Promise<Result<null, InternalError>> => {
    ...
    try {
        ...
        logger.error('database seeding failed', { error: String(e) });
        return err({ status: 500, code: 'INTERNAL_ERROR', message: 'Database seeding failed' });
    }
}